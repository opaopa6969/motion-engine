...
- `new Gesture(name, dur?, env?)` — `'tsumogiri' | 'headScratch' | 'fistPump' | 'slump' | 'recoil' | 'crossArms' | 'nod' | 'shrug' | 'lean' | 'smirkTilt' | 'sigh' | 'exhale' | 'headShakeRue' | 'ponder'`. `env` (v0.8) tunes anticipation/follow-through (`{windup, follow, anticipate, overshoot}`; `{windup:0}` = the plain bell). `ponder` here is the original Euler-delta gesture; `new ArmAct('ponder', geo)` is a separate IK-driven version that positions both arms from rig geometry.
...
- `ARM_ACTS` — the exported arm-acting vocabulary, including `clap`, `gutsPose`, `banzai`, `fistToForehead`, `ponder` and the 14 extra arm acts. The first four are available through `ArmAct`, not `Gesture`; `ponder` currently has both the IK-driven `ArmAct` form and the distinct Euler-delta `Gesture` form described above.
